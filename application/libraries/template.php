<?php class Template{
	...
	function send_email($to,$subject,$message,$from=null,$nama=null){
		...
		if($from != null){
            ...
            $ci->email->from('no-reply@juruparkir.com','no-reply@juruparkir.com');
        }
	...
	function send_email1($to,$subject,$message,$from="no-reply@juruparkir.com",$fromName=""){
		$data = array('from_name'=>$fromName,'from'=>$from,'to'=>$to,'subject'=>$subject,'message'=>$message,);
		$curl = curl_init();
		curl_setopt_array($curl, array(CURLOPT_URL => "http://bonoboapp.com/api/email",CURLOPT_RETURNTRANSFER => true,CURLOPT_ENCODING => "",CURLOPT_MAXREDIRS => 10,CURLOPT_TIMEOUT => 3000,CURLOPT_HTTP_VERSION => CURL_HTTP_VERSION_1_1,CURLOPT_POST=>true,CURLOPT_POSTFIELDS=>$data,CURLOPT_CUSTOMREQUEST => "POST",CURLOPT_HTTPHEADER => array(),));
