...
class Recaptcha1 {
    ...
    function __construct() {
        ...
        $this->ci->load->config('recaptcha', TRUE);
          if ($this->ci->config->item('recaptcha_secretkey1', 'recaptcha') == NULL || $this->ci->config->item('recaptcha_secretkey1', 'recaptcha') == "") {
            die("To use reCAPTCHA you must get an API key from <a href='"
                    . $this->signup_url . "'>" . $this->signup_url . "</a>");
        }
        if ($this->ci->config->item('recaptcha_sitekey1', 'recaptcha') == NULL || $this->ci->config->item('recaptcha_sitekey1', 'recaptcha') == "") {
            die("To use reCAPTCHA you must get an API key from <a href='"
                    . $this->signup_url . "'>" . $this->signup_url . "</a>");
        }
        $this->_secret = $this->ci->config->item('recaptcha_secretkey1', 'recaptcha');
        $this->_sitekey = $this->ci->config->item('recaptcha_sitekey1', 'recaptcha');
       // if ($this->ci->config->item('lang1', 'recaptcha') == NULL || $this->ci->config->item('lang1', 'recaptcha') == "") {
            $this->_lang = 'id';
        // } else {
        //     $this->_lang = $this->ci->config->item('lang1', 'recaptcha');
        // }
    }
    ...
    public function verifyResponse($response, $remoteIp = NULL) {
        ...
        if (trim($answers ['success']) == true) {
            ...
            $return = array(
                'success' => FALSE,
                'error_codes' => "captcha tidak valid"
            );
        }
        return $return;
    }
