...
function filter_top(id,jenis){
$.ajax({
	type : 'POST',
	data : 'id='+id+'&jenis='+jenis,
	url : base_url+'produk/filter_top',
	dataType: 'json',
	success : function (response){
		if(response.msg == 'sukses'){
			
			 window.location.href = base_url+"produk/index";
			 //$('#div-paging').html(atob(response.isi));
		}else{
...
function delete_table(url) {
	...
	if(values == ""){
        ...
        $(".body-delete > p").html("Tidak ada data yang dipilih");
       	$(".delete-cancel").hide(); 
        $(".delete-ok").hide();
    }else{
    	//$(".box-header").html("Konfirmasi");
...
function delete_table_tipe_lahan(url) {
	var values =   $('input:checkbox:checked.checkboxDelete').map(function (){
        return this.value;
    }).get();
	if(values == ""){
