...
$("#FormMenu").validate({
    ...
    var gambar_default_1 = $('#gambar_default_1').val();
      if (FormBiodata.valid()){
			$.ajax({
				type: 'POST',
				data: FormBiodata.serialize() + '&image_high_1='+image_high_1 + '&image_tumb_1='+image_tumb_1 + '&gambar_default_1='+gambar_default_1,
				url: base_url+'biodata/save_biodata',
       			success: function(msg) {
					if(msg == 1){
						$.ambiance({message: "Sukses",
            type: "success",
            fade: false});
					  top.location.href = base_url+'biodata';						
					}else{
...
function picture_upload_parking_edit(id,image_high,image_tumb,id_item){
  ...
  function cek_fasilitas(id){
    if ($('#cek_fasilitas'+id).is(":checked")) { 
       $('#fasilitas_name'+id).val('1');
  }else{
    $('#fasilitas_name'+id).val('0');
  }
  }

  function clik_aktif_mobil(){
    if ($('#aktif_mobil').is(":checked")) { 
       $('#active_mobil').val('1');
    }else{
      $('#active_mobil').val('0');
    }    
  ...
  function clik_aktif_motor(){
    if ($('#aktif_motor').is(":checked")) { 
       $('#active_motor').val('1');
    }else{
      $('#active_motor').val('0');
    }    
  ...
  function save_parking(){
    var form_add_parking = $('#form_add_parking');
    var jookir_idb = $('#jookir_idb').val();

      $.ajax({
        type: 'POST',
        data: form_add_parking.serialize(),
        url: base_url+'jookir/save_parking',
        success: function(response) {
          response=JSON.parse(response);
          if (response.msg == 'sukses') { 
            // $( ".tablink.tmobil" ).removeClass( "disabled" ).addClass( "fade in" );
            // $( ".tablink.tmotor" ).removeClass( "disabled" ).addClass( "fade in" );
            // $('.nav-tabs li a[href="#tab_2"]').tab('show');
            $.ambiance({message: "Sukses",
            type: "success",
            fade: false});
            $('#id_parking').val(response.id);
            top.location.href = base_url+'jookir/edit_parkir/'+jookir_idb;            
          }else{
                    $.ambiance({message: "Gagal",
                    type: "error",
                    fade: false});
